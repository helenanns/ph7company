...
if (has_post_thumbnail()) {
    ...
    if($thumbnail2) : 
        $thumbnail2 = $thumbnail2[1] ? wp_get_attachment_image_src($thumbnail2[1], 'medium_large') : null;
    endif;
}
?>
<article class="m-card-product<?php if ($status == 'outofstock') echo ' outofstock'; ?>">
    ...
    <a href="<?= get_the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
        ...
        <figure class="m-card__img" style="background-image: url('<?= esc_url($thumbnail[0]); ?>');">
            <?php 
                if($discount_percentage) :
                    echo '<span class="m-card__percentage">' . round($discount_percentage, 2) . '% OFF</span>';
                endif; 
            ?>
            <?php if (has_post_thumbnail()) : ?>
                
                <img src="<?= esc_url($thumbnail[0]); ?>" alt="<?php the_title(); ?>" loading="lazy" height="280" width="286">

                <?php if($thumbnail2) : ?>
                    <img src="<?= esc_url($thumbnail2[0]); ?>" alt="<?php the_title(); ?>" loading="lazy" height="280" width="286">
                <?php endif;?>
        ...
        <div class="m-card__content">
            ...
            <h3 class="m-title"><?= get_the_title(); ?></h3>
                            ...
                            <div class="m-card__price-value">
                                ...
                                <?php if ($sale_price) :
                                    ...
                                    <span class="old-price">
                                        <?php echo ' ' . explode(',', $regular_price)[0] . '<span>,' . explode(',', $regular_price)[1] . '</span>'; ?>
                                    </span>
                                <?php 
                                    $sale_price = number_format($sale_price, 2, ',', '.');
                                    echo 'R$ ' . explode(',', $sale_price)[0] . '<span>,' . explode(',', $sale_price)[1] . '</span>';
                                elseif ($regular_price) :
                                    $regular_price = number_format($regular_price, 2, ',', '.');
                                    echo 'R$ ' . explode(',', $regular_price)[0] . '<span>,' . explode(',', $regular_price)[1] . '</span>';
                                endif; ?>
